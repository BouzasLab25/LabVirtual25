...
ax[0].text(0, 0.42, 'Ruido', ha='center', va='bottom')    #Identificamos la Distribucion de Ruido con una etiqueta
ax[0].text(d, 0.42, 'Senal', ha='center', va='bottom')    #Etiqueta para la Distribucion de Senal
...
def update(adri):          # Creamos una funcion para actualizar nuestra grafica 
    ...
    ax[0].text(2.3, 0.61, 'FalsasAlarmas', color='#D3A519', ha='center', va='bottom', fontsize=10, fontweight='bold')   
    ax[0].text(4.05, 0.61, 'Hits', ha='center', color='#188A5B', va='bottom', fontsize=10)   
    ax[0].text(-1.6, 0.56, '(%.2f)' %scipy.stats.norm(0,1).cdf(k_), color='#C81BE2', ha='center', va='bottom', fontsize=10)     #Especificamos las áreas bajo la curva correspondientes a cada una: Rechazos correctos
    ax[0].text(0.2, 0.56, '(%.2f)' %scipy.stats.norm(d,1).cdf(k_), color='#003797', ha='center', va='bottom', fontsize=10)      #Omisiones
    ax[0].text(2.3, 0.56, '(%.2f)' %(1-scipy.stats.norm(0,1).cdf(k_)), color='#D3A519', ha='center', va='bottom', fontsize=10)  #Falsas alarmas
    ax[0].text(4.05, 0.56, '(%.2f)' %(1-scipy.stats.norm(d,1).cdf(k_)), ha='center', color='#188A5B', va='bottom', fontsize=10) #Hits
    ax[0].plot([k_,k_],[0,0.55], 'red') #Dibujamos el criterio
    ax[0].fill_between(yes, 0, (scipy.stats.norm(d,1).pdf(yes)), facecolor='#19D386', alpha=0.5)   #Coloreamos los Hits
    ax[0].fill_between(yes, 0, (scipy.stats.norm(0,1).pdf(yes)), facecolor='#F7C11D', alpha=0.5)   #Coloreamos las falsas alarmas
    ax[0].fill_between(no, 0, scipy.stats.norm(0,1).pdf(no), facecolor='#C81BE2', alpha=0.5)       #Coloreamos los Rechazos correctos
    ax[0].fill_between(no, 0, scipy.stats.norm(d,1).pdf(no), facecolor='#003797', alpha=0.5)        #Coloreamos las Omisiones
    ax[0].text(0, 0.42, 'Ruido', ha='center', va='bottom')       #Colocamos una etiqueta a la distribucion de Ruido   
    ax[0].text(d_, 0.42, 'Senal', ha='center', va='bottom')      #Colocamos una etiqueta a la distribucion de Senal
